...
async function loadChannels() {
  try {
    ...
    channelSelect.innerHTML = [
      ...
      normalized.length > 1 ? `<option value="__all__">All saved channels</option>` : "",
...
async function loadPosts({ append = false } = {}) {
  currentChannel = channelSelect.value === "__all__"
    ? "__all__"
    : document.querySelector("#channel").value.trim() || "TasnimNews";
  ...
  const translatePersian = document.querySelector("#translatePersian").checked;
  ...
  if (translatePersian) params.set("translate_search", "1");
  ...
  try {
    ...
    resultTitle.textContent = data.mode === "multi" ? "All saved channels" : `@${data.channel}`;
    const filterBits = [];
    if (data.mode === "multi" && data.channels) filterBits.push(`channels: ${data.channels.length}`);
    if (currentSearch) filterBits.push(`search: ${currentSearch.replace(/\n+/g, ", ")}`);
    if (data.translated_searches && data.translated_searches.length) {
      filterBits.push(`Persian: ${data.translated_searches.join(", ")}`);
    }
...
channelSelect.addEventListener("change", () => {
  ...
  if (channelSelect.value === "__all__") {
    channelInput.value = "All saved channels";
    channelInput.disabled = true;
  } else {
    channelInput.disabled = false;
    if (channelSelect.value) channelInput.value = channelSelect.value;
  }
});
